...
def index(request: Request):
    ...
    return templates.TemplateResponse(
        "index.jinja", {"request": request, "items": items}
    )
...
def list_(request: Request):
    items = fetch_all_watches()
    return templates.TemplateResponse(
        "_list.jinja", {"request": request, "items": items}
    )
...
def detail(request: Request, key: str):
    ...
    if result:
        item = Watch(**result)
        return templates.TemplateResponse(
            "detail.jinja", {"request": request, "item": item}
        )
...
def check(request: Request, key: str):
    ...
    if result:
        ...
        execute_watch(item)
        return templates.TemplateResponse(
            "_item.jinja", {"request": request, "item": item}
        )
    raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail="Item not found")
...
def update_snippet(request: Request, key: str):
    ...
    if result:
        item = Watch(**result)
        return templates.TemplateResponse(
            "_edit.jinja", {"request": request, "item": item}
        )
    raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail="Item not found")
...
def update(request: Request, key: str, data: WatchForm):
    if db.get(key):
        db.update(
            {**data.model_dump(), "updated_time": datetime.utcnow().timestamp()}, key
        )
